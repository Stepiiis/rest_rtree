...
public class RTree {
    ...
    public List<RTreeNode> kNN(Coordinate point, int k, kNNType implementation) {
        ...
        return switch (implementation) {
            case BRANCH_AND_BOUND -> kNNBranchAndBound(region, k);
...
//        return kNearest;
    ...
    public List<RTreeNode> kNNBranchAndBound(RTreeRegion queryRegion, int k) {
        PriorityQueue<RTreeNode> q = new PriorityQueue<>(Comparator.comparingDouble(val -> calculateMinDist(queryRegion, val.getMbr()))); // lowest distance to region stored first
        PriorityQueue<Pair<RTreeNode, Double>> result = new PriorityQueue<>(Comparator.comparing(Pair::getSecond, Comparator.reverseOrder()));
        q.add(getRoot());
    ...
    private void searchNode(RTreeNode child, double childDistance, int k, PriorityQueue<RTreeNode> q, PriorityQueue<Pair<RTreeNode, Double>> result) {
        if (db.isIndex(child.getId())) {
            q.add(db.getNode(child.getId()));
            return;
